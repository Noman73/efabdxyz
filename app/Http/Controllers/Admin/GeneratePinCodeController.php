...
class GeneratePinCodeController extends Controller
{
    ...
    {
      // return $get=DB::select("SELECT pin_builders.id,pin_builders.pin,users.name,if(user2.pin_no=null,0,user2.pin_no ) is_used from pin_builders
      ...
      // ");
        if (request()->ajax()){
            ...
            $get=DB::select("SELECT pin_builders.id,pin_builders.pin,users.name,if(user2.pin_no=null,0,user2.pin_no ) is_used from pin_builders
            inner join users on users.id=pin_builders.customer_id
            left join users as user2 on user2.pin_no=pin_builders.pin
            order by pin_builders.id
            ");
          ...
          ->addColumn('pin',function($get){
            return "<span class='p-1 text-white rounded ".($get->is_used==null ? "bg-success" :"bg-danger")."'>".$get->pin."<span>";
            })
